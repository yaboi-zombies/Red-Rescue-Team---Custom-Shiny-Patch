...
static bool8 IsAnyRecruitedPokemonShinyForSpecies(s16 species)
{
    s32 i;
    ...
    for (i = 0; i < NUM_MONSTERS; i++) {
        ...
        if (SpeciesId(pokemon->speciesNum) != SpeciesId(species))
            continue;

        if (pokemon->flags & POKEMON_FLAG_SHINY)
            return TRUE;
    }
        ...
        if (portraitPtr->showPortrait) {
            portraitPtr->faceFile = OpenPokemonDialogueSpriteFileForShiny(
                ...
                sTextboxPortraitIsShiny[portraitId] ||
                    IsAnyRecruitedPokemonShinyForSpecies(portraitPtr->speciesID)
            );
