...
OpenedFile *OpenPokemonDialogueSpriteFile(s16 index)
{
    ...
    /*
     * Compromise portrait behavior:
     *
     * 1. Preserve vanilla dialogue portraits when they exist.
     * 2. If no vanilla dialogue portrait exists, use generated runtime data.
     * 3. Prefer generated shiny data when available, then generated normal data.
     *
     * This avoids touching gMonsterFiles and avoids replacing existing vanilla
     * non-shiny dialogue portraits.
     */
    ...
    generatedFaceData = GetGeneratedDialogueSpriteDataPtr(index, TRUE);

    if (generatedFaceData == NULL)
        generatedFaceData = GetGeneratedDialogueSpriteDataPtr(index, FALSE);
...
OpenedFile *GetDialogueSpriteDataPtr(s32 index)
{
     ...
     * Compromise portrait behavior:
     *
     * Preserve vanilla dialogue portraits when they exist.
     * Only use runtime-generated dialogue portraits when vanilla has none.
     */
    ...
    if (generatedFaceData == NULL)
        generatedFaceData = GetGeneratedDialogueSpriteDataPtr(id, FALSE);
...
bool8 IsPokemonDialogueSpriteAvail(s16 index, s32 spriteId)
{
    // checking to see if dialogue sprite is available??
        /*
     * Runtime generated portrait slot availability.
     * Runtime generated portraits repeat Normal into every slot.
     */
    if (GetGeneratedDialogueSpriteDataPtr(index, TRUE) != NULL ||
        GetGeneratedDialogueSpriteDataPtr(index, FALSE) != NULL)
        return TRUE;
